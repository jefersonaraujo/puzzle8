...
function Busca() {
    ...
    // Verifica a pilhaAbertos ate encontrar o estado objetivo
    while (pilhaAbertos.length > 0) {
        var x

            x = pilhaAbertos.shift()

        // Compara com o estado meta pra saber se são iguais
        let objetivo = comparaEstados(x.estado, ob)

        // Se forem iguais, encontrou o estado igual o objetivo 
        if (objetivo) {
            console.log("Achei o objetivo")
            // Pega o estado encontrado e add no Arraysolução
            ...
            p_FilhosDeX.forEach(function (xp) {
                ...
                pilhaAbertos = pilhaAbertos.concat(filhosValidosDeX)
...
function empilharFilho(pai, k, l, m, n) {
    ...
    p_FilhosDeX.push({ estado: estado, pai: pai }); // Atribui o Nó na pilha filhos de X.
}
